...
async function grabStream(key: string): Promise<void> {
  busy.value = 'Starting…';
  try {
    const res = await ext.runtime.sendMessage({ kind: 'streams:start', key });
    busy.value = res?.ok ? 'Started' : (res?.error || 'Failed to start');
  } catch (e: any) {
  ...
  return (
    <div class="pop">
      ...
      {st.length ? (
        <div class="streams">
          {st.slice(0, 4).map((s) => (
            <button key={s.key} class="stream" onClick={() => void grabStream(s.key)}
                    title={`Grab — ${s.url}`}>
              <span class="stream-tag">{s.type}</span>
              <span class="stream-title">{s.title || s.url}</span>
            </button>
          ))}
        </div>
      ) : null}
